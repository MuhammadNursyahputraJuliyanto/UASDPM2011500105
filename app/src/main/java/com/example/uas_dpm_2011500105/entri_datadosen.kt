package com.example.uas_dpm_2011500105

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.widget.*

class entri_datadosen : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_entri_datadosen)
        ...
        val adpJbtn = ArrayAdapter(
            this@entri_datadosen,
            android.R.layout.simple_spinner_dropdown_item,
            Jabatan
        )
        spnJabatan.adapter = adpJbtn

        val Pangkat = arrayOf("III/a - Penata Muda",
        ...
        btnSimpan.setOnClickListener {
            if ("${etNIDN.text}".isNotEmpty() && "${etNmDosen.text}".isNotEmpty() &&
                ...
                val db = dbhelper(this@entri_datadosen)
                db.Nidn = "${etNIDN.text}"
